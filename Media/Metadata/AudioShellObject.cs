...
namespace YouTubeRipper.Media.Metadata
{
	...
	//		var duration = prop.FormatForDisplay(
		...
		: ShellRefObject
	{
		...
		public bool TrySetDate(
			DateTime dateTime)
		{
			try
			{
				using var shellPropertyWriter = ShellObject.Properties.GetPropertyWriter();
				shellPropertyWriter.WriteProperty(SystemProperties.System.GPS.Date, dateTime);
			...
			string title)
		{
			...
			{
				using var shellPropertyWriter = ShellObject.Properties.GetPropertyWriter();
				shellPropertyWriter.WriteProperty(SystemProperties.System.Title, title);
